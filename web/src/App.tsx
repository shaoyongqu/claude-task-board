...
export function App() {
      ...
      try {
        ...
        if (!policy) {
          throw new Error(textRef.current(
            "自动化服务没有返回实际生效的策略",
            "The automation service did not return the effective policy.",
          ));
        }
  ...
  useLayoutEffect(() => {
    ...
    if (pendingScroll.view === "issues" && boardScrollRef.current) {
      boardScrollRef.current.scrollLeft = pendingScroll.scrollLeft;
    }
  ...
  const trackedClaudeThreadIds = useMemo(() => [...new Set(tasks
    .filter((task) => task.status === "in_progress" && task.threadId)
    .map((task) => normalizeClaudeThreadId(task.threadId))
    .filter(Boolean))].sort(), [tasks]);
  const trackedClaudeThreadIdsKey = trackedClaudeThreadIds.join(",");

  useEffect(() => {
    if (trackedClaudeThreadIds.length === 0) {
      setClaudeSessionProgress({});
      return;
    }
    let disposed = false;
    const sync = async () => {
      try {
        const progress = await getClaudeSessionProgress(trackedClaudeThreadIds);
        if (!disposed) {
          setClaudeSessionProgress((current) => (
  ...
  function openThread(binding: CodexThreadBinding, title: string | null = null) {
    if (binding.codexProjectKind === "remote") {
      setActionError(text(
        ...
        "Open this SSH remote conversation in its original environment.",
      ));
      return;
    }
    openClaudeViewer(
      ...
      <main className="workspace">
        ...
        {detailTask && selectedProject ? (
          ...
          <div className="page-empty">
            ...
            <p>{text(
              "让 Claude Code 检查当前项目目录对应的会话，并整理任务状态。",
              "Ask Claude Code to inspect sessions for this project directory and organize their task status.",
            )}</p>
            <div className="page-empty-actions">
              <button
                ...
                onClick={() => setAiOpenThreadRequest((current) => ({
                  ...
                  issueId: null,
                  composerText: text(
                    "只检查当前项目目录对应的 Claude Code 会话。请将其中已完成、处理中和待执行的任务整理并导入当前项目的 Taskboard。",
                    "Only inspect Claude Code sessions associated with this project directory. Organize completed, in-progress, and pending tasks, then import them into this project's Taskboard.",
                  ),
                  requestId: (current?.requestId ?? 0) + 1,
                }))}
              >
