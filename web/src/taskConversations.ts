...
export function normalizeClaudeThreadId(value: string | null | undefined) {
...
export function taskConversations(task: Task, aiThreads: AiChatThread[]) {
  ...
  for (const ref of task.conversationRefs ?? []) {
    const normalizedId = normalizeClaudeThreadId(ref.threadId);
  ...
  for (const thread of aiThreads) {
    ...
    const normalizedId = normalizeClaudeThreadId(thread.claudeThreadId);
    ...
    const candidate: TaskConversationItem = {
      ...
      source: "local-ai",
      nativeThreadId: current?.nativeThreadId ?? thread.claudeThreadId,
...
export function taskCardPresentation(
  ...
  const normalizedRunningNativeThreadId = normalizeClaudeThreadId(runningNativeThreadId);
  const runningNative = task.status === "in_progress" && normalizedRunningNativeThreadId
    ? conversations.find((conversation) => (
        normalizeClaudeThreadId(conversation.nativeThreadId) === normalizedRunningNativeThreadId
      ))
    : undefined;
