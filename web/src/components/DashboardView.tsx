  ...
  const summaryBody = isAllProjects
    ...
    : projectSummary?.summary
      ?? (projectSummary?.refreshing
        ? text(
            "Claude Code 正在整理当前项目的进展、风险和下一步重点…",
            "Claude Code is reviewing the project's progress, risks, and next steps…",
          )
        : summaryLoadFailed || projectSummary?.error
          ? text("Claude Code 暂时无法生成项目总结。", "Claude Code cannot generate the project summary now.")
          : text(
              "Claude Code 正在整理当前项目的进展、风险和下一步重点…",
              "Claude Code is reviewing the project's progress, risks, and next steps…",
            ));
    ...
    <div className="dashboard-view">
      <div className="dashboard-content">
        <div className="dashboard-overview">
          ...
          <section className="dashboard-agent-summary" aria-label={text("Claude Code 项目总结", "Claude Code project summary")}>
            <div className="dashboard-summary-bubble">
              <p
                className={summaryTyping ? "is-typing" : undefined}
                aria-label={summaryReady ? summary : text("Claude Code 正在整理项目总结", "Claude Code is preparing the project summary")}
              >{displayedSummary}</p>
            </div>
            <img className="dashboard-agent-mark" src="claude-agent-logo.png" alt="" aria-hidden="true" />
          </section>
        </div>
