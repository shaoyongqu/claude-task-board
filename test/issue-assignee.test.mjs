...
test("issue model and editor preserve a concrete assignee identity", async () => {
  ...
  assert.match(typesSource, /export type AssigneeTarget = "current-user" \| "claude-agent"/);
  ...
  assert.match(editorSource, /CLAUDE_AGENT_ACTOR/);
});
...
test("issue detail and cards expose the same assignee identity", async () => {
  ...
  assert.match(avatarSource, /claude-agent-logo\.png/);
