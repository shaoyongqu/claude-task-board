...
async function startServer(configure, listenOptions = {}) {
  const directory = await mkdtemp(path.join(os.tmpdir(), "claude-task-board-test-"));
...
test("launcher mode proves service identity and hides every route behind its instance token", async () => {
  ...
  assert.equal(health.response.status, 200);
  assert.equal(health.body.product, "claude-task-board");
...
test("existing task and comment thread attribution remains content-specific", async () => {
  ...
  assert.equal(result.body.task.creatorId, "claude-agent");
  ...
  assert.equal(comments.body.comments[0].authorId, "claude-agent");
...
test("device workspaces come from this machine's Claude Code session projects", async () => {
  const baseUrl = await startServer(async (directory) => {
    const claudeHome = path.join(directory, "claude-home");
    for (const project of ["local-project-a", "local-project-b"]) {
      const sessionDirectory = path.join(claudeHome, "projects", project);
      await mkdir(sessionDirectory, { recursive: true });
      await writeFile(path.join(sessionDirectory, "session.jsonl"), JSON.stringify({
        type: "user",
        cwd: `/Users/alice/${project}`,
        sessionId: `00000000-0000-0000-0000-0000000000${project.length}`,
  ...
  assert.deepEqual(result.body.workspaces, {
    "local-project-a": "/Users/alice/local-project-a",
    "local-project-b": "/Users/alice/local-project-b",
  });
});
...
test("trusted HTTPS origins allow their exact public Host without trusting forwarded headers", async () => {
  ...
  const baseUrl = await startServer(async (directory) => {
    ...
    return {
      ...
      processEnv: { ...process.env, CLAUDE_TASKBOARD_TRUSTED_ORIGINS: trustedOrigin },
    };
  });
...
test("trusted HTTPS origins do not inherit device-local capabilities from tunnel loopback", async () => {
  ...
  assert.equal((await request(baseUrl, "/api/local/claude-session-progress?threadId=00000000-0000-0000-0000-000000000001")).response.status, 200);
...
test("trusted HTTPS origins apply to cloud WebSocket upgrades without widening loopback routes", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "claude-task-board-trusted-origins-"));
  const trustedOrigin = "https://board.example.test";
...
test("trusted origin configuration rejects non-origin URLs", () => {
  const valid = resolveServerOptions({
    processEnv: {
      ...process.env,
      CLAUDE_TASKBOARD_TRUSTED_ORIGINS: "https://board.example.test, https://second.example.test/",
    },
  });
  ...
  for (const value of [
    ...
    assert.throws(
      () => resolveServerOptions({
        processEnv: { ...process.env, CLAUDE_TASKBOARD_TRUSTED_ORIGINS: value },
      }),
      /CLAUDE_TASKBOARD_TRUSTED_ORIGINS/,
    );
  }
});
...
test("an explicit thread binding is persisted verbatim on task writes", async () => {
  const baseUrl = await startServer();
  const binding = {
    threadId: "local-thread",
    codexProjectId: "/work/local-project",
  ...
  const task = (await request(baseUrl, "/api/tasks", {
    method: "POST",
    body: { title: "Local binding", threadId: "local-thread", threadBinding: binding },
  })).body.task;
...
test("taskctl issue creation and comments use the Claude Agent identity", async () => {
  ...
  assert.equal(task.creatorId, "claude-agent");
