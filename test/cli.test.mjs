...
async function run(argv, fetchImplementation, overrides = {}) {
  ...
  const exitCode = await main(argv, {
    ...
    env: { CLAUDE_THREAD_ID: "thread-current" },
    ...overrides,
  });
  return {
    exitCode,
...
test("CLAUDE_TASKBOARD_URL overrides the service origin", async () => {
  let requestedUrl;
  const result = await run(
    ...
    { env: { CLAUDE_TASKBOARD_URL: "https://tasks.example.test/" } },
  );
...
test("CLAUDE_TASKBOARD_WSL_RUNTIME_FILE overrides WSL automatic discovery", async () => {
  ...
  const result = await run(
    ...
    undefined,
    {
      env: {
        ...
        CLAUDE_TASKBOARD_WSL_RUNTIME_FILE: runtimeFile,
      },
      execFile: async () => {
      ...
      spawn: (file, args) => {
        ...
        queueMicrotask(() => {
          ...
          child.stderr.end("__CLAUDE_TASKBOARD_CURL_RESPONSE__200\tapplication/json\t15");
          child.emit("close", 0);
        });
        return child;
      },
...
test("an explicit --thread-id overrides CLAUDE_THREAD_ID on issue writes", async () => {
  let requestBody;
  const result = await run(
...
test("issue and comment writes require Codex conversation attribution", async () => {
  ...
  assert.match(issueResult.stderr.error.message, /--thread-id or CLAUDE_THREAD_ID/);
  ...
  assert.match(commentResult.stderr.error.message, /--thread-id or CLAUDE_THREAD_ID/);
});
