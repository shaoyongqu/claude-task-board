...
description: Manage Claude Task Board / e-taskboard work with taskctl. Use for taskboard issue IDs, status sync, comments, or taskctl configuration - not for unrelated product docs.
---
...
- When no binary is injected, run `node <taskboard-repo>/cli/taskctl.mjs` from this board's repository, or use `taskctl` if it is on PATH via `npm link`. Point it at the active board with `CLAUDE_TASKBOARD_URL` when needed; do not guess another port or data source.
...
In this product, **companion** means the **device-local loopback service** used for cloud mode (Claude Code/Git/Skill, path mapping, Basic Auth proxy). Related names: `local companion`, `loopback companion`, `CLAUDE_TASKBOARD_COMPANION_URL`, `cloud-companion.json`, `LOCAL_COMPANION_REQUIRED`.
...
- Let `taskctl` read `CLAUDE_THREAD_ID` for controller attribution; the board injects it into every Claude Code session it drives. Outside a board-driven session, pass the exact conversation ID with `--thread-id`. This value alone is not a complete task binding.
- Any issue that the current conversation claims or continues must store a complete `threadBinding`: `threadId`, `codexProjectId`, `codexProjectKind`, `codexHostId`, and `workspacePath`. For an unbound local issue launched with injected Taskboard context, use the current `CLAUDE_THREAD_ID`, the injected project id (the workspace path for this board), `local` project kind, and `local` host id. Pass all five explicit `--binding-*` options on the claim and every later `issue move` that retains ownership. If any identity field is unavailable, stop before moving the issue to `in_progress`; never create a legacy binding containing only `threadId`.
