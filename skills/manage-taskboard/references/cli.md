...
**Companion** here is a product term for the **device-local loopback HTTP service** that `taskctl` talks to in cloud mode. It applies Basic Authentication, stores device-only project path mappings, and keeps Claude Code/Git/Skill/MCP capabilities on the machine. It is not a chat persona and not a separate public “companion product API”.
...
Env and files that refer to this service: `CLAUDE_TASKBOARD_COMPANION_URL`, `CLAUDE_TASKBOARD_URL` (loopback origin), `.data/cloud-companion.json`. Error code `LOCAL_COMPANION_REQUIRED` means a capability needs that **local loopback service**, not a different API surface.
...
Set `CLAUDE_TASKBOARD_URL` to override the default local API origin, `http://127.0.0.1:47823`.
...
Every issue or comment write must be attributed to a Claude Code session. In a board-driven session, `taskctl` reads the current session from `CLAUDE_THREAD_ID`. Outside a session, pass `--thread-id ID` explicitly. An explicit option takes precedence over the environment. Read commands do not require a conversation id.
...
Issues created through `taskctl` are assigned to Claude Agent by default. Other CLI writes preserve the existing assignee.
...
Use either `--git-branch` or `--worktree-path`/`--worktree-branch`; an issue has only one development context. Issue JSON stores it as `developmentContext`, either `{ "type": "branch", "branch": "..." }` or `{ "type": "worktree", "path": "...", "branch": "..." }`. Its singular `threadId` is the Claude Code session that most recently created or changed the issue itself. Recurrence requires a due date.

Changing only `--project` preserves the issue's existing linked conversation.

## Issue relations

Read the anchor issue immediately before adding or removing a relation and use its current version. Relation writes require Claude Code session attribution like every other issue write.
