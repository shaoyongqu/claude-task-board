...
切换项目 → 创建项目 → 输入项目名称，**工作目录**（workspace path）可在创建时直接填写，之后随时修改：在切换项目菜单中**右键该项目 → 设置工作目录**（必须是本机已存在的绝对路径，如 `D:\projects\my-repo`）。AI 对话和自动化都在该目录下驱动 Claude Code 工作；未设置时 AI 面板和自动化菜单会提示补设。
