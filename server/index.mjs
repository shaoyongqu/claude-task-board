...
async function main() {
  ...
  const listenFd = process.env.CLAUDE_TASKBOARD_LISTEN_FD === undefined
    ? null
    : Number(process.env.CLAUDE_TASKBOARD_LISTEN_FD);
  const address = await app.listen({ host, port: resolvePort(), fd: listenFd });
  console.log(`Claude Task Board listening on http://127.0.0.1:${address.port}`);
  if (host === "0.0.0.0") {
    ...
    for (const lanAddress of [...new Set(addresses)]) {
      console.log(`Claude Task Board available on LAN at http://${lanAddress}:${address.port}`);
    }
  }
