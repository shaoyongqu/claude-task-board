...
const CLAUDE_SESSION_TAIL_BYTES = 16 * 1024 * 1024;
...
function parseAssigneeTarget(value) {
  if (value === undefined) return undefined;
  if (value !== "current-user" && value !== "claude-agent") {
    throw new ApiError(400, "INVALID_FIELD", "'assigneeTarget' must be current-user or claude-agent");
  }
  return value;
}
...
async function readClaudeSessionDirectory(claudeHome) {
  try {
    return await readdir(path.join(claudeHome, "projects"), { withFileTypes: true });
  } catch {
    return [];
  }
}

async function findClaudeSessionFile(claudeHome, sessionId) {
  for (const entry of await readClaudeSessionDirectory(claudeHome)) {
    if (!entry.isDirectory()) continue;
    const candidate = path.join(claudeHome, "projects", entry.name, `${sessionId}.jsonl`);
    try {
...
async function readClaudeProjectWorkspaces(claudeHome) {
  ...
  for (const entry of await readClaudeSessionDirectory(claudeHome)) {
    if (!entry.isDirectory()) continue;
    const projectDirectory = path.join(claudeHome, "projects", entry.name);
    let newest = null;
    try {
      const files = await readdir(projectDirectory, { withFileTypes: true });
      for (const file of files) {
        ...
        if (fileStat && (!newest || fileStat.mtimeMs > newest.mtimeMs)) {
          newest = { path: filePath, mtimeMs: fileStat.mtimeMs };
        }
      }
    } catch {
      continue;
    ...
    try {
      ...
      for (const line of buffer.toString("utf8", 0, bytesRead).split("\n")) {
        ...
        if (typeof record?.cwd === "string" && record.cwd.trim()) {
          workspaces[entry.name] = record.cwd.trim();
          break;
        }
        break;
      }
...
export function resolveServerOptions(options = {}) {
  ...
  const configuredDataDirectory = options.dataDirectory ?? environment.CLAUDE_TASKBOARD_DATA_DIR;
  ...
  const instanceToken = String(
    options.instanceToken ?? environment.CLAUDE_TASKBOARD_INSTANCE_TOKEN ?? "",
  ).trim();
  if (instanceToken && !/^[a-z0-9-]{16,128}$/i.test(instanceToken)) {
    throw new Error("CLAUDE_TASKBOARD_INSTANCE_TOKEN must be an identifier");
  }
  const instanceSecret = String(
    options.instanceSecret ?? environment.CLAUDE_TASKBOARD_INSTANCE_SECRET ?? "",
  ).trim();
  if (instanceToken && !/^[a-f0-9-]{32,128}$/i.test(instanceSecret)) {
    throw new Error("CLAUDE_TASKBOARD_INSTANCE_SECRET must be set in launcher mode");
  }
  return {
    dataDirectory,
    ...
    skillPath: options.skillPath
      ?? environment.CLAUDE_TASKBOARD_SKILL_PATH
      ?? path.join(PROJECT_ROOT, "skills", "manage-taskboard", "SKILL.md"),
    claudeExecutable: resolveClaudeExecutable({ explicit: options.claudeExecutable }),
    claudeHome,
    instanceToken,
    instanceSecret,
    trustedOrigins: parseTrustedOrigins(environment[TRUSTED_ORIGINS_ENV]),
    version: String(
      options.version ?? environment.CLAUDE_TASKBOARD_VERSION ?? "development",
    ).trim(),
  };
}

export function resolvePort(value = process.env.CLAUDE_TASKBOARD_PORT ?? "47823") {
  const port = typeof value === "number" ? value : Number(value);
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error("CLAUDE_TASKBOARD_PORT must be an integer between 1 and 65535");
  }
  return port;
}

export function resolveHost(value = process.env.CLAUDE_TASKBOARD_HOST ?? "0.0.0.0") {
  const host = String(value).trim();
  if (host !== "127.0.0.1" && host !== "0.0.0.0") {
    throw new Error("CLAUDE_TASKBOARD_HOST must be 127.0.0.1 or 0.0.0.0");
  }
  return host;
}
...
export function createTaskboardServer(options = {}) {
  ...
  function currentHostThreadBinding() {
    return undefined;
  }
  ...
  const aiChat = new AiChatService({
    ...
    models: configuredModels(options.processEnv ?? process.env),
  ...
  async function findClaudeSession(threadId) {
    ...
    const sessionPath = await findClaudeSessionFile(resolved.claudeHome, threadId);
    claudeSessionSearches.set(threadId, { path: sessionPath, checkedAt: Date.now() });
    return sessionPath;
  }
  ...
  async function readClaudeSessionState(threadId) {
    ...
    if (lastRecord?.type === "assistant" && typeof lastRecord.timestamp === "string") {
      ...
      running = Number.isFinite(lastTimestamp)
        ...
        && sessionStat.mtimeMs > Date.now() - 2 * 60 * 1_000;
    }
    ...
    for (let index = records.length - 1; index >= 0; index -= 1) {
      ...
      const todoBlock = record.message.content.find((block) => (
        block?.type === "tool_use" && block.name === "TodoWrite" && Array.isArray(block.input?.todos)
      ));
      if (!todoBlock) continue;
      const statuses = todoBlock.input.todos
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (pathname === "/api/local/automation") {
        if (request.method !== "POST") return methodNotAllowed(response, ["POST"]);
        const body = await readJson(request);
        const parsed = parseTaskboardAutomationHostRequest(body);
        if (!parsed) {
      ...
      if (pathname === "/api/local/claude-session-progress") {
        if (request.method !== "GET") return methodNotAllowed(response, ["GET"]);
      ...
      if (pathname === "/api/device-workspaces") {
        ...
        return sendJson(response, 200, {
          workspaces: await readClaudeProjectWorkspaces(resolved.claudeHome),
        });
      }
