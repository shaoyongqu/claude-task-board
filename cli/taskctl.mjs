...
async function execute(parsed, overrides) {
  ...
  const env = parsed.options["runtime-file"] === undefined
    ? processEnv
    : { ...processEnv, CLAUDE_TASKBOARD_RUNTIME_FILE: parsed.options["runtime-file"] };
  const usesCompanionControl = command.startsWith("cloud ") || command === "project map";
  const target = usesCompanionControl || env.CLAUDE_TASKBOARD_COMPANION_URL !== undefined
...
function resolveThreadId(options, overrides) {
  const env = overrides.env ?? process.env;
  const value = options["thread-id"] ?? env.CLAUDE_THREAD_ID;
  if (typeof value !== "string" || value.trim().length === 0) {
    throw usageError("Claude Code conversation attribution requires --thread-id or CLAUDE_THREAD_ID");
  }
  const threadId = value.trim();
  if (threadId.length > 256) {
    throw usageError("--thread-id and CLAUDE_THREAD_ID cannot exceed 256 characters");
  }
  return threadId;
}
...
function normalizeBaseUrl(rawUrl) {
  ...
  try {
    ...
    throw usageError("CLAUDE_TASKBOARD_URL must be a valid URL");
  }
  if (url.protocol !== "http:" && url.protocol !== "https:") {
    throw usageError("CLAUDE_TASKBOARD_URL must use http or https");
  }
...
async function resolveTaskboardBaseUrl(env, overrides) {
  if (env.CLAUDE_TASKBOARD_URL !== undefined) {
    return { url: env.CLAUDE_TASKBOARD_URL, windowsTransport: false };
  }
  const configuredDescriptorPath = env.CLAUDE_TASKBOARD_RUNTIME_FILE;
  const isWsl = isWslEnvironment(env);
  const wslRuntimeFile = env.CLAUDE_TASKBOARD_WSL_RUNTIME_FILE;
...
async function fetchThroughWindows(url, init, overrides) {
  ...
  const marker = "__CLAUDE_TASKBOARD_CURL_RESPONSE__";
  const args = [
    "--disable",
    "--noproxy",
    "*",
...
async function resolveCompanionUrl(env, overrides) {
  const target = env.CLAUDE_TASKBOARD_COMPANION_URL !== undefined
    ? { url: env.CLAUDE_TASKBOARD_COMPANION_URL, windowsTransport: false }
    : await resolveTaskboardBaseUrl(env, overrides);
  let url;
  try {
